...
from .forms import IndividualForm, SampleForm, SampleForm2, SampleMultiForm, UploadFileForm, FullSampleForm, FreezerForm, BoxForm, SampleFormHuman, SampleMultiFormHuman
from .models import Individual, Sample, Sample2, Freezer, FreezerPos, Box, BoxPos
from .tables import IndividualTable, SampleTable, FullSampleTable, FreezerTable, BoxTable, FullSampleTable2
...
def add_full_sample2_single(request):
    title = "Enter Individual and Sample2"
    single_url = 'add_full_sample_single'
    multi_url = 'add_full_sample_multiple'
    p_form = SampleFormHuman(prefix="ind")
    c_form = SampleMultiForm(prefix="samp")
    table = FullSampleTable2(Sample2.objects.all())

    # if request.POST:
    #     p_form = SampleFormHuman(request.POST, prefix="ind")
...
def add_sample2_multiple(request):
    title = 'Enter Multiple Samples2'
    single_url = 'add_sample_single'
    multi_url = 'add_sample_multiple'
    extras = 5
    pks = list()
    qset = Sample2.objects.none()
    data = ()
    ...
    SampleFormSet = modelformset_factory(Sample2, form=SampleForm2, extra=extras)
    p_formset = SampleFormSet(queryset=qset, initial=data)
    c_formset = 'n/a'
    table = SampleTable(Sample2.objects.all())
