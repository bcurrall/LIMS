...
class GenericUpdateFormSet(CreateView):
    # class defaults
    initial_data = []
    extra = 0
    upload_form = UploadFileForm()
    ...
    def get_context_data(self, *args, **kwargs): #gets context for building formset
        ...
        formset = self.get_formset()
        # queryset = self.model.objects.none()
        print('============get_context_data pks===============')
        print(self.pks)
        queryset = self.model.objects.filter(pk__in=['1'])
        context['formset'] = formset(queryset=queryset, initial=self.initial_data)
        context['title'] = self.title
        context['button_type'] = self.button_type
        context['basic_url'] = self.basic_url
    ...
    def get(self, request, *args, **kwargs): #handles all posts
        print('=================get====================')
        print(request.GET)
        self.object = None
        context = self.get_context_data()
    ...
    def post(self, request, *args, **kwargs): #handles all posts
        ...
        pks = request.POST.getlist("selection")
        qset = self.model.objects.filter(pk__in=pks)
        print('============pks==============')
        print(pks)
        ...
        if "save_btn" in self.request.POST:
            ...
            print('=========else pks=============')
            print(pks)
            context = self.get_context_data(pks=pks)
            return render(request, self.template_name, context=context)
